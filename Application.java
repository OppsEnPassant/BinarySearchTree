...
// Prompt from GPT, for the whole 'split' method
    public Node[] split(int key, Node root) {
        Node[] result = new Node[2];
        Node[] splitNodes = splitRecursive(key, root);
        result[0] = splitNodes[0];
        result[1] = splitNodes[2];
        return result;
    }

    private Node[] splitRecursive(int key, Node root) {
        if (root == null) {
            Node[] empty = {null, null, null};
            return empty;
        }

        if (key < root.id) {
            Node[] leftSplit = splitRecursive(key, root.left);
            root.left = leftSplit[1];
            leftSplit[1] = root;
            return leftSplit;
        } else if (key > root.id) {
            Node[] rightSplit = splitRecursive(key, root.right);
    ...
    public static void main(String[] args) {
        ...
        Node[] splitNodes = bst.split(60, bst.getRoot());
        BinarySearchTree leftTree = new BinarySearchTree();
        leftTree.setRoot(splitNodes[0]);
        BinarySearchTree rightTree = new BinarySearchTree();
        rightTree.setRoot(splitNodes[1]);
